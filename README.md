...
Besides loading games and core options, the menu rebinds Player 1's keyboard (Settings > Keyboard) and the gamepad buttons for every port (Settings > Gamepad, or `SDL_Libretro_SetGamepadMapping()` from code), swaps disks for multi-disk games (Disks), and shows core-reported progress messages.
