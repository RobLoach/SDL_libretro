...
To enable the in-app menu, enable the `SDL_LIBRETRO_MENU` CMake option (linking the `SDL_libretro_menu` target), and let SDL_libretro know it's available with `SDL_LIBRETRO_ENABLE_MENU`. It brings Load Game, save states, core options, controller selection, and an Audio & Video settings page (volume, mute, fullscreen, vsync, filter, fit mode, themes and UI scale), navigable with keyboard, mouse or gamepad. Toggle it with `F1` or the gamepad Guide button. The UI scales automatically with the window resolution and display scale, with a UI Scale setting (Auto, 1x–4x) to override it. The theme and UI scale persist through the config file when one is initialized, and on the web the Load Game button opens the browser's file picker.
